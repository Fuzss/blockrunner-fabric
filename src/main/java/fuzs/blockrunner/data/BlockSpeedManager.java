...
public class BlockSpeedManager implements PreparableReloadListener {
    ...
    public void onPlayerLoggedIn(ServerGamePacketListenerImpl handler, PacketSender sender, MinecraftServer server) {
        if (ModLoaderEnvironment.isServer()) {
            BlockRunner.NETWORK.sendTo(new S2CBlockSpeedMessage(this.serialize(this.customBlockSpeedValues)), handler.player);
        }
    }
        ...
        for (Map.Entry<String, JsonElement> entry : jsonElements.entrySet()) {
            ...
            if (key.startsWith("#")) {
                ...
                if (Registry.BLOCK.containsKey(resourcelocation)) {
                    ...
                    throw new JsonSyntaxException("Unknown block type '" + resourcelocation + "', valid types are: " + Joiner.on(", ").join(Registry.BLOCK.keySet()));
                }
            }
        }
